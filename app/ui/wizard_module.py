...
class ModuleWizard(QWidget):
    ...
    def _build_ui(self):
        ...
        hdr.setStyleSheet(f"background:qlineargradient(x1:0,y1:0,x2:1,y2:0,"
                           f"stop:0 {self.MODULE_COLOR}20,stop:1 #020617); border-bottom:2px solid {self.MODULE_COLOR};")
        hlay = QHBoxLayout(hdr); hlay.setContentsMargins(15,0,15,0)
        self.title_lbl = QLabel(f"⚙ {self.MODULE_NAME}")
        self.title_lbl.setStyleSheet(f"color:{self.MODULE_COLOR};font-size:16px;font-weight:900;")
        hlay.addWidget(self.title_lbl); hlay.addStretch()
        self.step_counter = QLabel("")
        self.step_counter.setStyleSheet("color:#64748b;font-size:12px;")
        ...
        self.progress.setStyleSheet(f"QProgressBar{{background:#1e293b;border:none;}}"
                                     f"QProgressBar::chunk{{background:{self.MODULE_COLOR};}}")
        ...
        for w in self.step_widgets:
            ...
            scroll.setStyleSheet("QScrollArea{border:none;background:#020617;} QScrollBar{background:#0f172a;}")
            self.stack.addWidget(scroll)

        # Pantalla final (generar PDF)
        self.stack.addWidget(self._build_final_screen())
        root.addWidget(self.stack, stretch=1)

        # ── Navegación ──
        nav = QFrame(); nav.setFixedHeight(60)
        nav.setStyleSheet("background:#0f172a;border-top:1px solid #1e293b;")
        nlay = QHBoxLayout(nav); nlay.setContentsMargins(20,8,20,8); nlay.setSpacing(12)

        self.btn_prev = QPushButton("← Anterior")
        self.btn_prev.setStyleSheet("background:#1e293b;color:#94a3b8;border:1px solid #334155;"
        ...
        self.btn_next.setStyleSheet(f"background:{self.MODULE_COLOR};color:#020617;"
    ...
    def _build_final_screen(self):
        ...
        lay = QVBoxLayout(w); lay.setAlignment(Qt.AlignmentFlag.AlignCenter); lay.setSpacing(20)
        lay.addStretch()
        ico = QLabel("✅"); ico.setStyleSheet("font-size:64px;"); ico.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lay.addWidget(ico)
        lay.addWidget(_lbl("¡Análisis completado!", "#10b981", True, 22))
        lay.addWidget(_lbl(f"Todos los pasos del módulo {self.MODULE_NAME} han sido procesados.",
                           "#64748b", size=13))
        lay.addWidget(_sep())
        btn_pdf = QPushButton("📄  GENERAR REPORTE PDF")
        btn_pdf.setStyleSheet(f"background:{self.MODULE_COLOR};color:#020617;"
                               f"border-radius:8px;padding:14px 30px;font-weight:900;font-size:15px;")
    ...
    def _update_nav(self):
        ...
        for i in range(n + 1):
            if i == self.current_step:
                dots += f"<span style='color:{self.MODULE_COLOR};font-size:18px;'>●</span> "
            else:
                dots += "<span style='color:#1e293b;font-size:18px;'>●</span> "
